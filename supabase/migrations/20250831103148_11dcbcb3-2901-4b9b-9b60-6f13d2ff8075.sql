...
-- 6. Add data masking function for sensitive personal information
...
-- 7. Create secure user profile access function
...
BEGIN
  ...
  IF target_user_id != requesting_user_id AND get_user_role(requesting_user_id) != 'admin' THEN
  ...
  SELECT 
    u.id,
    CASE 
      WHEN requesting_user_id = u.id OR get_user_role(requesting_user_id) = 'admin' 
      THEN u.email
      ELSE mask_personal_data(u.email, u.id, 'email', requesting_user_id)
    END,
    u.full_name,
    CASE 
      WHEN requesting_user_id = u.id OR get_user_role(requesting_user_id) = 'admin'
      THEN u.phone
      ELSE mask_personal_data(u.phone, u.id, 'phone', requesting_user_id)
    END,
    u.role,
    u.is_vip,
    u.kyc_status,
...
-- 8. Enhanced security event logging
