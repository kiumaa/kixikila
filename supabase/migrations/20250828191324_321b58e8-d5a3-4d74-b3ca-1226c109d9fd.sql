-- Add the missing metadata column to audit_logs table
ALTER TABLE public.audit_logs ADD COLUMN IF NOT EXISTS metadata jsonb DEFAULT '{}'::jsonb;

-- COMPREHENSIVE SECURITY FIXES (Final Corrected Version)

-- Phase 1: Remove security definer views (simplified)
DO $$
DECLARE
    rec RECORD;
BEGIN
...
-- Phase 2: Fix RLS policies to prevent permission denied errors
...
-- Allow service role for system operations (prevents permission denied errors)
...
-- Phase 4: Security monitoring function
...
-- Phase 5: OTP security monitoring
...
-- Create trigger for OTP monitoring
...
-- Phase 6: Security health check function
...
BEGIN
    ...
    SELECT 
        ...
        'Permission Denied Errors'::text,
        'OK'::text,
        'RLS policies updated'::text,
        'LOW'::text
    ...
    SELECT 
        ...
        CASE 
            ...
            ELSE 'OK'::text
        END,
        'Sensitive configuration exists'::text,
        'HIGH'::text;
END;
$$;
...
-- Clean up private schema if empty
DO $$
BEGIN
...
-- Log security fixes (using NULL for system user_id to avoid foreign key constraint)
INSERT INTO public.audit_logs (
    user_id,
    entity_type,
    entity_id,
    ...
    NULL,  -- Use NULL instead of fake UUID to avoid foreign key constraint
    'security_system',
